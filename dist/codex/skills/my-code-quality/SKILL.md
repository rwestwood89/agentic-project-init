---
name: my-code-quality
description: Run code quality checks and fix issues systematically. Use when you want tests, lint, or validation work driven to closure.
---

Generated from `claude-pack/commands/_my_code_quality.md`. This is a command-derived Codex skill. Rebuild it instead of editing it by hand.
...
- Before quality check → ``my-implement`` (complete feature first)
- After quality check → Commit clean code, create PR
- If major issues found → ``my-design`` or ``my-plan`` (may need design changes)
