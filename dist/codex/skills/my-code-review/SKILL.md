---
name: my-code-review
description: Review implementation against requirements and design. Use when you want a bug-focused audit or requirements check.
---

Generated from `claude-pack/commands/_my_code_review.md`. This is a command-derived Codex skill. Rebuild it instead of editing it by hand.

# Code Review Command

**Purpose:** Audit implementation against spec`my-design` requirements, identify gaps, and find potential bugs
**Input:** Spec`my-design` reference OR general review prompt
...
1. **Determine Review Type**:
   - **Spec-Based Review**: Feature name provided → review against spec`my-design`
...
**Related Commands:**
- After implementation → ``my-code-review`` to verify
- Before code-review → ``my-implement`` should be complete
- After code-review with issues → Fix issues, then re-review
- After code-review passes → ``my-code-quality`` for automated checks, then ``my-project-manage``
