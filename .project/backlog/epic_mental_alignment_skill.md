...
- The sibling-file mechanics on **both** runtimes are answered, by a probe run against throwaway
  ...
  prefixed directory skills register on both. Reference a sibling by **bare filename in prose** and
  resolve it from the runtime-supplied skill locator. Claude prepends its base directory; Codex
  supplies the absolute `SKILL.md` path in the skills inventory.
...
3. **ADR reference.** ADRs 0009 (directory-skills pattern) and 0011 (native-skill Codex adapter) are already filed; the spec for the packaging item should reference them rather than filing new ones. This observation originally named 0010, which the owner reversed on 2026-08-20; 0011 supersedes it.
...
- Write every sibling reference as a **bare filename in prose** — `design_synthesis.md`, `feedback/synthesis.md` — never as a path containing the skill's own directory name. On Claude, join it to the prepended `Base directory for this skill`. On Codex, join it to the directory containing the absolute `SKILL.md` path supplied in the skills inventory. Evidence: `.project/active/directory-skill-build-pattern/spike-findings.md`, A8 and corrected B5; `.project/active/directory-skill-build-pattern/cwd-spike-findings.md`. Bare-filename prose is a preference where it fits, not a cwd constraint
...
- **The Codex-side unknowns are resolved — no probe needed in this item** (`.project/active/directory-skill-build-pattern/spike-findings.md`, B1-B8; corrected cwd result in `.project/active/directory-skill-build-pattern/cwd-spike-findings.md`). Codex loads a symlinked skill *directory*, but silently refuses to register a skill whose `SKILL.md` is a symlink. It reads flat, twice-nested, and non-markdown siblings, and tolerates stray files with no warning. Skill activation preserves the project/session working root, and the skill reaches siblings from its supplied `SKILL.md` path. Codex accepts underscore-prefixed names. It registers the **frontmatter `name`**, not the directory name — which kills the renamed-symlink simplification and makes D1's generated entry point mandatory rather than convenient. The derived Codex name must land in the generated frontmatter; `build-codex-pack.sh:386` already writes it, so the work here is to not simplify it away
...
| A skill can't read its sibling files, or the reference form is cwd-dependent | **Resolved** | Siblings are readable on both runtimes — flat, nested, and through a symlink (`spike-findings.md`, A6-A9, B4-B5). The correction probe (`cwd-spike-findings.md`) confirms skill activation preserves the project/session cwd. Resolve siblings from the runtime-supplied skill locator; project-relative output paths remain project-relative. |
